...
const ParentRow = ({ parent, onClick, showRemove, onRemove }) => {
  const name =
    parent.name || `${parent.user?.fname || ''} ${parent.user?.lname || ''}`.trim() || '—';
...
const AddLearnerModal = ({
  ...
  const handleAddParent = (p) => {
    if (!linkedParents.some((lp) => lp.user_id === p.user_id))
      setLinkedParents((prev) => [...prev, p]);
  };
  const handleRemoveParent = (uid) =>
    setLinkedParents((prev) => prev.filter((p) => p.user_id !== uid));

  const isValid =
    formik.values.learner_id &&
        ...
        <Box component="form" onSubmit={formik.handleSubmit}>
          <Box sx={{ mb: 3 }}>
            <TextField
              ...
              fullWidth
              required
              error={formik.touched.learner_id && !formik.values.learner_id}
              helperText={formik.touched.learner_id && !formik.values.learner_id ? 'Admission number is required' : ''}
            />
          </Box>
            ...
            <>
              ...
              {parentResults.length > 0 && (
                <Box sx={{ mb: 1.5 }}>
                  ...
                  >
                    {parentResults.length} result{parentResults.length !== 1 ? 's' : ''} — click to
                    link
                  </Typography>
                  <Box
                    sx={{
                    ...
                    {parentResults.map((parent) => {
                      ...
                      return (
                        <Box key={parent.user_id} sx={{ position: 'relative' }}>
                          <ParentRow
                            parent={parent}
                            onClick={!alreadyLinked ? () => handleAddParent(parent) : undefined}
                            showRemove={false}
                          />
                          {alreadyLinked && (
                            <Chip
