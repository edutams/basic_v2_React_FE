...
const ViewParentsContent = ({ learner, onClose }) => {
  const learnerName = learner?.users ? `${learner.users.fname} ${learner.users.lname}` : 'Learner';
  ...
  useEffect(() => {
    ...
    setLoading(true);
    learnerApi
      .getParents(learner.users.id)
      .then((res) => setParents(res?.data?.data ?? []))
      .catch(() => setError('Failed to load parents.'))
      .finally(() => setLoading(false));
  }, [learner?.users?.id]);

  const relationshipLabel = (rel) => (rel ? rel.charAt(0).toUpperCase() + rel.slice(1) : '—');
