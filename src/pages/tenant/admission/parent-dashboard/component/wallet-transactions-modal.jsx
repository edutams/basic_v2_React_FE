...
import { getParentWalletTransactions } from '@/api/tenant/admission/admissionApi';

const naira = (n) => `₦${(Number(n) || 0).toLocaleString()}`;

/**
 * "Wallet Transactions" on the Parent Wallet Account card — this
 * guardian's own wallet balance plus the bursary payments settled from it.
 * Mirrors the bursar-facing WalletTransactions.jsx page's content, just as
 * a modal fed by the parent-scoped endpoint instead of the bursar one.
 */
const WalletTransactionsModal = ({ open, onClose }) => {
  ...
  const load = useCallback(async (pageNumber) => {
    ...
    setError('');
    try {
      const res = await getParentWalletTransactions(pageNumber + 1);
      if (res?.status) {
