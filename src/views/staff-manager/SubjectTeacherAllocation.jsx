...
const SubjectTeacherAllocation = () => {
  ...
  const handleProgrammeChange = async (progId) => {
    ...
    if (progId) {
      try {
        ...
        fetchAllocations(progId, selectedClassArm);
      } catch (error) {
  ...
  const handleClassArmChange = (classArmId) => {
    ...
    if (selectedProgramme && classArmId) {
      fetchAllocations(selectedProgramme, classArmId);
    }
  };

  const fetchAllocations = async (progId, classArmId = null) => {
    setLoading(true);
    try {
      const params = {
        programme_id: progId,
        ...
        <Table sx={{ border: '1px solid #e0e0e0' }}>
          <TableHead sx={{ bgcolor: '#fafafa' }}>
            <TableRow>
              ...
              <TableCell sx={{ fontWeight: 700, border: '1px solid #e0e0e0' }}>Subject Name</TableCell>
