 ...
 *  - My Wards          ← /admission/parent-dashboard wards, enriched with
 *                        attendance % (parent/attendance) and average score
 *                        (parent/academics) per ward.
 *  - Parent Wallet     ← finance.outstanding from the dashboard payload.
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      getParentNotifications(),
        ...
        const attendance = results[1]?.status === 'fulfilled' ? results[1].value : null;
        const academics = results[2]?.status === 'fulfilled' ? results[2].value : null;
        const notifications = results[3]?.status === 'fulfilled' ? results[3].value : null;

        if (dash?.status) {
          setDashboardData(dash.data);
          if (dash.data?.wards?.length > 0) {
            setSelectedWard(dash.data.wards[0]);
          }
        }
    ...
    <PageContainer title="Parent Dashboard" description="Parent Portal Overview">
      ...
      >
        ...
        <Box sx={{ flex: '1 1 0', minWidth: 0, width: { xs: '100%', lg: 'auto' } }}>
          {/* 1. My Wards Section (3 cards max per view with Prev & Next icons) */}
          <MyWards
            wards={wards}
            loading={loading}
        ...
        <Box sx={{ width: { xs: '100%', lg: 320 }, flexShrink: 0 }}>
          {/* 1. Parent Wallet Account Card */}
          <ParentWalletAccount
            totalPayable={dashboardData?.finance?.outstanding ?? 0}
          />

          {/* 2. Term Calendar Card */}
