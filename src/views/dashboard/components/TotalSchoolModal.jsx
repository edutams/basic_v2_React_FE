...
const SchoolsOverviewModal = ({ open, onClose, stats }) => {
  ...
  const overviewChartOptions = {
    ...
    xaxis: {
      ...
      labels: {
        rotate: 0,
        style: { fontSize: '12px', fontWeight: 500, colors: theme.palette.mode === 'dark' ? '#fff' : '#333' },
      },
    ...
    >
      <Box>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 2, sm: 3 }} mb={3}>
          ...
          >
            <Box sx={{ width: 48, height: 48, borderRadius: '50%', background: theme.palette.primary.light, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <IconSchool size={22} color={theme.palette.primary.main} />
            </Box>
            <Box>
              <Typography fontSize={26} fontWeight={700}>{stats?.totalSchools ?? 0}</Typography>
              <Typography fontSize={14} color="text.secondary">Total School</Typography>
            </Box>
          </Paper>
          ...
          >
            <Box sx={{ width: 48, height: 48, borderRadius: '50%', background: '#FDE4E4', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        >
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'stretch', sm: 'center' }} justifyContent="space-between" mb={2}>
            ...
            >
              <Tab value={0} icon={<IconLayoutDashboard size={18} />} iconPosition="start" label="Overview" sx={{ textTransform: 'none', minHeight: 42 }} />
              <Tab value={1} icon={<IconUsers size={18} />} iconPosition="start" label="Agent Performance" sx={{ textTransform: 'none', minHeight: 42 }} />
            </Tabs>
          ...
          >
            {chartLoading ? (
              ...
              <Box width="100%">
                <Chart options={overviewChartOptions} series={[{ name: 'Schools', data: overviewSeries }]} type="bar" height={300} />
              </Box>
            ) : (
              <Box width="100%">
                {agentCategories.length === 0 ? (
                  ...
                  <Chart options={agentChartOptions} series={[{ name: 'Schools', data: agentSeries }]} type="bar" height={300} />
                )}
              </Box>
            )}
          </Box>
