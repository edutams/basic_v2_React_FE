...
export const academicInfoValidationSchema = Yup.object({
  ...
  prev_school_state: Yup.mixed().when('has_previous_school', {
    is: true,
    then: (s) => s.required('State is required'),
    otherwise: (s) => s.nullable(),
  }),
  prev_school_lga: Yup.mixed().when('has_previous_school', {
    is: true,
  ...
  intending_programme_id: Yup.mixed().required('Programme is required'),
  intending_class_id: Yup.mixed().required('Class choice is required'),
