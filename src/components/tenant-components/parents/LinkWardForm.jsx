...
const LinkWardForm = ({ parent, onSave, onCancel }) => {
  ...
  useEffect(() => {
    ...
    setSearch(''); setClassId(''); setResults([]);
    guardianApi.getWards(parent.user_id)
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      setSearching(true);
      const res = await guardianApi.searchLearners({ search: search.trim(), class_id: classId || undefined });
  ...
  const handleAdd    = (l) => { if (!linkedWards.some((w) => w.id === l.id)) setLinkedWards((p) => [...p, l]); };
  ...
  const WardRow = ({ ward, onClick, showRemove }) => {
    ...
    return (
      ...
      >
        ...
        {showRemove && (
          <IconButton size="small" color="error" onClick={(e) => { e.stopPropagation(); handleRemove(ward.id); }}>
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', gap: 1, mb: 2, p: 1.5, bgcolor: 'grey.50', borderRadius: 2, alignItems: 'center' }}>
        ...
        <TextField
          size="small" placeholder="Search Learner ID | Learner Name" value={search}
            ...
            {results.map((learner) => {
              ...
              return (
                <Box key={learner.id} sx={{ position: 'relative' }}>
                  <WardRow ward={learner} onClick={!alreadyLinked ? () => handleAdd(learner) : undefined} showRemove={false} />
                  {alreadyLinked && (
