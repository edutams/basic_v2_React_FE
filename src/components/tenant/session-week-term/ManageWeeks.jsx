  ...
  const handleGenerateWeeks = async () => {
    ...
    try {
      const response = await tenantApi.post(
        `/session-mappings/${sessionTermId}/weeks/auto-generate`,
        {
  ...
  return (
    <Box>
      ...
      <Dialog open={deleteDialog.open} onClose={() => setDeleteDialog({ open: false, week: null })}>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete <strong>{deleteDialog.week?.week_name}</strong>? This
            will remove the week and its dates from this term.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
