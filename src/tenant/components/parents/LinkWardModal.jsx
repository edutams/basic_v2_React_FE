...
const LinkWardModal = ({ open, onClose, parent, onSaved }) => {
  ...
  const parentName = parent?.user ? `${parent.user.fname} ${parent.user.lname}` : 'Guardian';
  ...
  return (
    <ReusableModal open={open} onClose={onClose} title={title} size="medium">
      <LinkWardForm key={parent?.user_id} parent={parent} onSave={handleSave} onCancel={onClose} />
    </ReusableModal>
  );
};
